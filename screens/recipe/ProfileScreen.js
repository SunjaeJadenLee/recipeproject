    ...
    <View style={{...styles.container,backgroundColor:darkModeColor}}> 
      ...
      <ScrollView> 
          ...
          <FlatList 
            ...
            contentContainerStyle={{width:375*REM,paddingHorizontal:10*REM,flex:1}}
            data={recipes}
            renderItem={({ item, index }) => <RecipeBox navigation={navigation} index={index} length={data.length} item={item}/>}
            // ListFooterComponent={()=><RecipeBox navigation={navigation} isFooter={true}/>}
          />
      </ScrollView> 
    </View>
  );
